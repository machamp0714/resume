...
- **技術的に工夫したこと**
インフラの構築を Terraform を導入する前はマネジメントコンソールから手動で行っていたので、
似た構成であっても毎回同じ手順で構築する必要がありました。そこに課題を感じ、インフラをコード化し、
構築を自動化するために Terraform の導入を決断しました。アプリケーションの開発同様 Github Flow に
基づいて、インフラの開発を進められる環境を整えました。

- **成果**
開発上の要件以外にも管理画面に対して IP 制限を掛ける、ログを7年間保存する等、セキュリティチェックシートへの
...
- **成果**
エンジニアとして初めて参画したプロジェクトですが要件定義から開発及びインフラの構築まで自分で主導し、
...
- **技術的に工夫したこと**
スクレイピングや外部のサービスに依存しており、バッチがネットワークエラー等が原因で
定期的に失敗してしまうことがありましたが、Slack API を利用して Slack 上でバッチを
再実行出来る仕組みを構築しました。

- **成果**
顧客が100を超える Web サイトにログインし、 CSV をダウンロードし、スプレッドシートへ
移管するという業務を全て自動化する事が出来ました。また例えバッチが失敗したとしても、
